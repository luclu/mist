...
gulp.task('upload-binaries', (cb) => {
    ...
    got(`https://api.github.com/repos/luclu/mist/releases?access_token=${GITHUB_TOKEN}`, { json: true })
        ...
        if (draft.draft === true) {
            ...
            .then(() => {
                if (draft.body && checksums) {
                    got.patch(`https://api.github.com/repos/luclu/mist/releases/${draft.id}?access_token=${GITHUB_TOKEN}`, {
                        body: JSON.stringify({
                            body: `${draft.body}\n\n## Checksums\n\`\`\`\n${checksums.join('')}\`\`\``
                        })
                    });
